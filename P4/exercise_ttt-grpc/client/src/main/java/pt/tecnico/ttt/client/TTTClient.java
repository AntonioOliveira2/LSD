		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			do {
				...
				do {
					...
					if (go == 10) {
						stub.refazer(RedoRequest.newBuilder().setPlayer(player).setPos(go).build());
						continue;
					}
					...
					try {
						play_res = stub.play(
								PlayRequest.newBuilder().setRow(row).setColumn(column).setPlayer(player).build()).getResult();
